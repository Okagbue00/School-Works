...
# This function is a black box.  Under no circumstances are you to edit it.
...
zip_song='music.zip' #zips the song
music='music' #music file
zip_result='zip-result.txt' #zips the result of the music text


Music_file(){
  IFS=$'\n'
  read -a RELEASE_YEAR_COL <<< "$1"

#is quiet and does not write anything to stdout
if grep -q '^[0-1]*$' - <<< ${RELEASE_YEAR_COL[ENTRY_NUMBER-1]}
then

    ARTIST_NAME=${RELEASE_YEAR_COL[ARTIST_COL- 1]}
    ARTIST=${ARTIST}


    ALBUM_TYPE_NAME=${RELEASE_YEAR_COL[ALBUM_TYPE_COL - 1]
    ALBUM_NAMEFILE=${ALBUM_TYPE_NAME}

    FEAT_ARTIST_COL=${RELEASE_YEAR_COL[FEAT_ARTIST_COL -1]}

    if [ $FEAT_ARTIST_COL == 'None' ]; then
        FEAT_ARTIST_COL=0
    else
      FEAT_ARTIST_COL=1
    fi
...
mkdir -p $music
cd $music #changes the directory
...
#for i in $(cat $Text_File)
#do



#going to compress the file and zip it
zip -r $zip_song > $zip_result
